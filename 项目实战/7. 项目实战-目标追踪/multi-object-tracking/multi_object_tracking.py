...
# "mosse": 最小输出平方误差和跟踪器 Minimum Output Sum of Squared Error (MOSSE) (cv2.legacy.TrackerMOSSE_create)。
...
# 创建一个多目标跟踪器，后续通过trackers.add()来新增，通过trackers.update()来更新。
...
while True:
    # 取当前帧
    frame = vs.read()
    # 第一个参数，也就是frame[0]代表视频是否被成功读取，成功为True；第二个参数标识读取到的帧画面。
    frame = frame[1]
    # 到头了就结束
    if frame is None:
        break
    ...
    (success, boxes) = trackers.update(frame)  # 刚开始没有add，update就获取不到东西。
